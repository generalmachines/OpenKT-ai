...
4. Pack turns in order into chunks of at most `maxChars` (default 6000) counting `content.length`; never split a turn unless that single turn exceeds `maxChars`, in which case split it on blank lines, then on sentence ends, into parts that fit, keeping `seq` and adding `part: n` in a new optional field. No part may exceed `maxChars` — a run of blank lines longer than `maxChars` is hard-split too.
5. Each chunk after the first gets `overlap` = the last `overlapTurns` (default 2) turns of the previous chunk. `overlapTurns: 0` means `overlap: []` and no `[context — already processed]` line (careful: `slice(-0)` returns the whole array).
...
- [ ] Empty input → `[]`.
- [ ] `overlapTurns: 0` → chunk 2 has `overlap: []` and its text has no `[context — already processed]` line.
- [ ] One turn of 7,000 newlines → every part ≤ 6,000 chars and the parts join back to the original.
- [ ] typecheck and tests pass.
...
- [ ] With `a`, `c` ideas and `b` a decision, `capFacts([a, b, a, c], 3)` → `[a, b, a]`.
- [ ] typecheck and tests pass.
...
3. `guardSupersede(newFact: { kind: Kind; created_at: string; project_id: string; owner_user_id: string }, agentAnswer: { duplicate_of: string | null; supersedes: string[] }, candidates: Neighbour[]): { duplicate_of: string | null; supersedes: string[]; rejected: { id: string; reason: string }[] }` applying every guard in Spec 02 §3. De-duplicate the ids first. Each rejected id gets one reason: not in `candidates` → `unknown_id`; either `created_at` unparseable by `Date.parse` → `bad_date`; target not strictly older, comparing `Date.parse` values and never the strings (offsets and milliseconds are legal) → `not_older`; other project → `other_project`; pinned or other author unless new kind ∈ {decision, fact, how-to} → `protected_target`. Count only the ids that passed every guard: more than 3 → `supersedes` is empty and each of them is rejected `suspicious_supersede`. If `duplicate_of` is valid, `supersedes` is forced empty.
...
- [ ] 4 valid supersedes → all rejected as `suspicious_supersede`; 4 ids of which 1 is unknown → the 3 valid ones kept; `['a','a']` → `['a']`.
- [ ] New fact `2026-06-01T00:00:00Z`: target `2026-05-31T23:30:00-05:00` → `not_older`; `2026-06-01T05:00:00+07:00` → kept; `not a date` → `bad_date`.
- [ ] `decideDuplicate` with similarities `[NaN, 0.99]` → duplicate of the 0.99 neighbour.
...
- [ ] 6 proposed → 4 returned, no duplicates; with an empty vocab `created` equals the 4 returned.
- [ ] `'a'×31 + '-' + 'b'×18` (50 chars, the dash is character 32) → `'a'×31`.
- [ ] `'a' + '𠀀'×40` → `'a' + '𠀀'×31` (no half character).
- [ ] Thai `ราคา ลูกค้า` → `ราคา-ลูกค้า`; Hindi `ग्राहक मूल्य` → `ग्राहक-मूल्य`; `a^b` → `a-b`.
- [ ] typecheck and tests pass.
...
- [ ] A locked target becomes `Updates` with the flag set; if `Updates` is locked too → unrouted `locked`.
- [ ] A `noop` proposal → unrouted `noop`.
- [ ] `null` fields behave like absent ones: `new_page` with `new_page_title: null` → `bad_title`.
- [ ] typecheck and tests pass.
...
- [ ] `fallbackAppend` output always passes `validateSection` when under the length limit — tested with statements ending in `?` and `!`, containing `. `, and containing a newline.
- [ ] A list line such as `1. Deploy the API [^f:<id>]` is one cited sentence.
...
- [ ] `speakerKnown: false` gives 0.50 with `quoteTurnRole` `speaker`, `user` and `assistant`.
...
**On hold — do not start.** A maintainer is changing desktop packaging and the macOS workflow (`.github/workflows/desktop.yml`, `apps/desktop/electron-builder.yml`). This task will be rewritten to cover only what is left — the auto-update wiring in `apps/desktop/src/main/updater.ts` — and the hold lifted then.
