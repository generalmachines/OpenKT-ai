...
| Sessions (T0) | `modules/sessions/` · tables **`kt_sessions`, `kt_session_turns`** · sources are one list, `SESSION_SOURCES` in `contracts/session.contract.ts` · `(owner, source, external_id)` is unique (idempotent create) · turns append under a row lock; closed → 409 `session_closed` |
| MCP tool text | `modules/mcp/services/mcp-text.ts` — every core tool returns readable text + `structuredContent` (Spec 04) |
...
| Who can see what | `modules/access/access-scope.service.ts`, `libs/auth/authorization/src/access-policy.ts` · search authorises every id in `filters.project_ids` (one unreadable id → 404 for the whole request); the workspace ring (`ProjectScopeService.workspaceRing`) holds only spaces the caller can read on their own · recall/search with **no space** search every readable space + every granted session through `access/readable-sql.ts` — subqueries inside the ranking SQL that must match `requireProjectAccess(read)` · the personal space is `projects.is_personal` (one per owner), never guessed |
