...
| Recall (hybrid search, fusion, rerank hook) | `modules/memory/services/memmachine-memory-engine.service.ts` (`search()`), `memory-recall.service.ts` |
...
Rules that bite:
- **Never name a table `sessions`.** Databases that ever ran the old MemMachine component already have one. Ours are `kt_sessions` and `kt_session_turns`.
...
- The class `MemMachineMemoryEngine` is, despite its name, the local Postgres engine. MemMachine itself is off by default (`OPENKT_MEMORY_ENGINE=local`).
