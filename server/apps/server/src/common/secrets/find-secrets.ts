// COPY of packages/pipeline/src/secrets.ts — the reviewed secrets filter
// (tasks J6 #53 and J6b #65). The server is not an npm workspace, so it cannot
// import @openkt/pipeline; it carries this copy instead. Do not edit below the
// marker: change packages/pipeline/src/secrets.ts, then paste it here again.
// test/unit/find-secrets-sync.spec.ts fails whenever the two differ.
// Used by ./refuse-secrets.ts on every save path (Spec 04: saves refuse secrets).
